...
class Base:
    ...
    def __init__(self, log_text=""):
        self.path = os.getcwd()
        self.log = self.path + "/pipe-translator.log"
        self.counter = 0
        self.file_list = []
...
class PepsObj(NtxtObj):
    ...
    def translate_to_peps_file(self):
        ...
        for item in self.element_cards:
            if self.ntxt_dataframe["type"].iloc[item] == "ELMT":
                ...
                peps_dataframe.loc[item,"WT"] = self.ntxt_dataframe["thick"].iloc[item]
    ...
    def write_peps(self):
        ...
        for i, row in peps_dataframe.iterrows():
            if row["IDEN"] == "CROS":
                if peps_dataframe["OD"].loc[i+1] != 0:
                    if peps_dataframe["WT"].loc[i+1] != 0:
                        self.append_peps("CROS CD=" + str(row["CD"]))
                        self.append_peps(" OD=" + str(peps_dataframe["OD"].loc[i+1]))
                        self.append_peps(" WT=" + str(peps_dataframe["WT"].loc[i+1]))
                        self.append_peps("\n")
